...
\begin{equation} \label{eq:transferfunctt3}
P(z)=\frac{\vec{e}(k)}{\vec{a}(k)}   
\end{equation}

where $P(z)$ is described as the transfer function of the LPC filter and $H(z)$ in the equations \ref{eq:transferfunct2} and \ref{eq:transferfunctt3} is the LPC Analysis filter.
...
where DFT is described as the discrete Fourier transformation and computed via \emph{fft}$(x,N)$ function in Matlab with $N=2800$ samples in our case and RMS is defined as:
...
\section{Speed Test}
In the following, we use a Novint Falcon haptic device to feel a surface with different friction conditions, to be able to create a speed scale for some friction coefficients. Our aim is to demonstrate how user speed varies depending on friction coefficients. We then use these results to evaluate speed intervals to be created in the following of this thesis, in which different audio data is displayed.
