...
As depicted in Figure \ref{fig:experiment2}, best result was achieved in the interaction with coak material with 100 \% similarity rate. Granite tile and steel wool have the same rate namely 90 \%, where fine foam has 80 \% and coarse foam 50 \% similarity rate. Eventually, overall percentage of similarity is 82 \% in 5 comparison pairs with 10 subjects.  

\subsection{Discussion}
We ran a study that evaluated the realism of the signals produced from major frequencies in respect to speed responsiveness. The result 82 \% similarity rate to the reference shows promise of this method's usability for vibrotactile response. 50 \% rate of coarse foam can be explained by the fact that original signal may have some pecularities occurred in the beginning and end of the recording, which could result in discontinuity in vibrotactile response during interaction. Another reason can be a lack of the used method, which 
